...
func GenerateToFileFromFile(
	...
	err = Generate(&generated, typ.Type().(*types.Named), dstPkgName, dstImportPath, dstTypeName)
	if err != nil {
...
func Generate(w io.Writer, typ *types.Named, pkgName, importPath, rename string) error {
...
`)
	...
	g := &generator{
		...
		cmpPkg:     imports.addIfNotPresent("cmp", "github.com/google/go-cmp/cmp"),
		fmtPkg:     imports.addIfNotPresent("fmt", "fmt"),
		runtimePkg: imports.addIfNotPresent("runtime", "runtime"),
	}

	if orderedImps := imports.ordered(); len(orderedImps) > 0 {
